...
## (merged p1_*/p2_* devices — one person per machine), slots 2–4 use per-peer
## REMOTE intent bitmasks received by Net. The simulation reads the same snapshot API
...
func capture() -> void:
	...
	if net != null and net.is_host():
		for slot in SLOTS:
			var pressed_mask: int = net.consume_remote_pressed(slot) if slot != 1 else 0
			for a in ACTIONS:
				var action := "p%d_%s" % [slot, a]
				if slot == 1:
					_held[action] = (Input.is_action_pressed("p1_" + a) or Input.is_action_pressed("p2_" + a)) and not suppress_local
					_pressed[action] = (Input.is_action_just_pressed("p1_" + a) or Input.is_action_just_pressed("p2_" + a)) and not suppress_local
				else:
					_held[action] = net.remote_held(slot, a)
					_pressed[action] = NetCodec.mask_has(pressed_mask, a)
		return
	for slot in SLOTS:
		for a in ACTIONS:
			var action := "p%d_%s" % [slot, a]
