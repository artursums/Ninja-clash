...
func run() -> void:
	...
	for index in modes.tiles.size():
		...
		check(not modes._choice_modal.visible, "Browsing modes does not expose a hidden difficulty shortcut")
	modes._choose(2)
	unlock(modes)
	modes._continue()
	unlock(modes)
	modes._confirm_choice(2)
	...
	unlock(modes)
	modes._continue()
	check(modes._choice_open and state.current_state == state.State.MODE_SELECT, "Solo requires an explicit rank choice")
	check(modes.tiles[0].disabled, "Rank dialog blocks mode cards behind it")
	modes._choose_option(3)
	unlock(modes)
	modes._continue()
	...
	clans._mouse_pick(0)
	clans._mouse_lock(2)
	check(not clans.selection.ready[1], "Two players cannot lock the same clan")
