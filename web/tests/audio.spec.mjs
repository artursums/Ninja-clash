...
test('browser music and menu effects produce audio through separate buses', async ({ page }) => {
  ...
  const base = process.env.NINJA_AUDIO_TEST_URL || process.env.NINJA_TEST_URL || (process.env.NINJA_TEST_TURN ? 'http://127.0.0.1:8788' : 'http://127.0.0.1:8787');
  await page.goto(base);
  await expect.poll(() => state).toBe('WELCOME');
  ...
  await click(400, 342);
  await expect.poll(peak, { timeout: 8000,
    message: 'Music must reach the browser audio output after interaction' }).toBeGreaterThan(0.01);
