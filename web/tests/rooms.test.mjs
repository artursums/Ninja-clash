...
const create = rooms => rooms.handle({ action: 'create', token: host, protocol: 5 });
const join = (rooms, room, token = guest) => rooms.handle({ action: 'join', token, room, protocol: 5 });
...
test('invalid signaling, tokens, and rate exhaustion are rejected', async () => {
  const { rooms } = setup();
  await assert.rejects(rooms.handle({ action: 'create', token: 'guess', protocol: 5 }), { status: 400 });
  const { room } = await create(rooms);
  await join(rooms, room);
...
test('HTTP boundary rejects cross-origin requests and never exposes service secrets', async () => {
  ...
  const body = { action: 'create', token: host, protocol: 5 };
