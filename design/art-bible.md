...
> **Author**: art-director agent + orchestrator
...
- **Pillars** (`design/gdd/game-pillars.md`): Visual pillars align with Restraint over Spectacle, Fairness Is Sacred, Game-Feel First.
- **Game Concept** (`design/gdd/game-concept.md`): Clan names, mechanical identity, scope tier.
- **Launch Map Specs** (`design/levels/launch-maps.md`): Map count, theme assignments, tile palette skin requirements feed back into this document.
- **Combat GDD** (`design/gdd/combat.md`): VFX trigger events (catch, hit, death) sourced from combat signals.
- **Movement / Player** (`design/gdd/movement.md` if exists, `prototypes/movement-and-combat/player.gd`): Animation state list derived from player state machine.
