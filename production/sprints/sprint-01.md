...
| T3 | **ADR-0001**: input/state separation (keeps online v2 viable; enables testable input) | You + `technical-director` | 0.5 | — | ADR in `docs/architecture/`; specifies input→intent→state boundary GSM + Couch Input must follow |
| T4 | **Game State Manager** to GDD (Boot→Menu→Setup→InMatch→MatchEnd) + GUT tests | You + `godot-gdscript-specialist` | 1.0 | T2, T3 | All states + transitions per GDD; `state_changed` signal; unit tests cover every transition; meets GDD acceptance criteria |
| T5 | **Couch Input** to GDD: 2-controller + keyboard, slot assignment, signals | You + `godot-specialist` | 1.5 | T2, T3, T4 | Slot assignment works; hot-plug handled; emits `throw_pressed`/`pause_requested` etc. per GDD; tests for slot logic |
