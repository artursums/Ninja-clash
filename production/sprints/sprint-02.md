...
| T4 | **Movement — core verbs**: reconcile `movement.md` with the prototype-validated feel decisions, then implement walk, jump, and dodge (i-frames) on CC, driven by `CouchInput.get_intent(slot)` + `primary_action_pressed`; pin tuned values | You + `gameplay-programmer` + `game-designer` | 1.5 | T3, T1 | GDD updated for validated changes (single jump / no double jump, dash up-height cap, instant ground-reset on landing; defense remains deferred). Character walks, jumps, and dodges in `src/` via intent; dodge i-frame window unit-tested against the pinned value. Movement reads intent only (ADR-0001). |
